...
# Đường dẫn tới các tệp màu pywal và cấu hình Yazi
WAL_COLORS="$HOME/.cache/wal/colors"
YAZI_THEME="$HOME/.config/yazi/theme.toml"

# Kiểm tra xem tệp màu của pywal có tồn tại không
if [[ ! -f "$WAL_COLORS" ]]; then
    echo "Pywal colors not found!"
    exit 1
fi

# Đọc các màu từ tệp pywal
readarray -t colors < "$WAL_COLORS"

# Cập nhật theme.toml của Yazi
cat > "$YAZI_THEME" <<EOF
[manager]
cwd = { fg = "${colors[7]}" }

hovered         = { fg = "${colors[0]}", bg = "${colors[5]}" }
preview_hovered = { fg = "${colors[0]}", bg = "${colors[6]}" }

find_keyword  = { fg = "${colors[4]}", italic = true }
find_position = { fg = "${colors[8]}", bg = "reset", italic = true }

marker_copied   = { fg = "${colors[2]}", bg = "${colors[2]}" }
marker_cut      = { fg = "${colors[3]}", bg = "${colors[3]}" }
marker_marked   = { fg = "${colors[7]}", bg = "${colors[7]}" }
marker_selected = { fg = "${colors[5]}", bg = "${colors[5]}" }

tab_active   = { fg = "${colors[0]}", bg = "${colors[6]}" }
tab_inactive = { fg = "${colors[6]}", bg = "${colors[9]}" }
tab_width    = 1

count_copied   = { fg = "${colors[0]}", bg = "${colors[2]}" }
count_cut      = { fg = "${colors[0]}", bg = "${colors[3]}" }
count_selected = { fg = "${colors[0]}", bg = "${colors[5]}" }

border_symbol = "│"
border_style  = { fg = "${colors[10]}" }

syntect_theme = "~/.config/yazi/Catppuccin-mocha.tmTheme"

[mode]
normal_main = { fg = "${colors[0]}", bg = "${colors[5]}", bold = true }
normal_alt  = { fg = "${colors[5]}", bg = "${colors[8]}"}

select_main = { fg = "${colors[0]}", bg = "${colors[2]}", bold = true }
select_alt  = { fg = "${colors[2]}", bg = "${colors[8]}"}

unset_main  = { fg = "${colors[0]}", bg = "${colors[10]}", bold = true }
unset_alt   = { fg = "${colors[10]}", bg = "${colors[8]}"}

[status]
separator_open  = ""
separator_close = ""

progress_label  = { fg = "${colors[6]}", bold = true }
progress_normal = { fg = "${colors[12]}", bg = "${colors[9]}" }
progress_error  = { fg = "${colors[3]}", bg = "${colors[9]}" }

perm_type  = { fg = "${colors[12]}" }
perm_read  = { fg = "${colors[4]}" }
perm_write = { fg = "${colors[3]}" }
perm_exec  = { fg = "${colors[2]}" }
perm_sep   = { fg = "${colors[10]}" }
...
desc            = { fg = "${colors[12]}" }
separator       = "  "
separator_style = { fg = "${colors[10]}" }

[help]
on      = { fg = "${colors[7]}" }
run     = { fg = "${colors[12]}" }
desc    = { fg = "${colors[9]}" }
hovered = { bg = "${colors[10]}", bold = true }
footer  = { fg = "${colors[6]}", bg = "${colors[9]}" }

[notify]
title_info  = { fg = "${colors[7]}" }
title_warn  = { fg = "${colors[4]}" }
title_error = { fg = "${colors[3]}" }

[filetype]
rules = [
    { mime = "image/*", fg = "${colors[7]}" },
    { mime = "{audio,video}/*", fg = "${colors[4]}" },
    { mime = "application/*zip", fg = "${colors[12]}" },
    { mime = "application/x-{tar,bzip*,7z-compressed,xz,rar}", fg = "${colors[12]}" },
    { mime = "application/{pdf,doc,rtf}", fg = "${colors[2]}" },
    { name = "*", fg = "${colors[6]}" },
    { name = "*/", fg = "${colors[5]}" }
]
...
[icon]
files = [
  { name = "kritadisplayrc", text = "", fg = "${colors[5]}" },
  { name = ".gtkrc-2.0", text = "", fg = "${colors[12]}" },
  { name = "bspwmrc", text = "", fg = "${colors[8]}" },
  { name = "webpack", text = "󰜫", fg = "${colors[10]}" },
  { name = "tsconfig.json", text = "", fg = "${colors[10]}" },
